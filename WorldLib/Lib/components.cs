...
namespace Remouse.Shared.EcsLib.LeoEcsLite {
    ...
    internal interface IEcsAutoReset<T> where T : struct {
        void AutoReset (ref T c);
    }

    internal interface IEcsAutoCopy<T> where T : struct {
...
#endif
    internal sealed class EcsPool<T> : IEcsPool where T : struct {
        readonly Type _type;
