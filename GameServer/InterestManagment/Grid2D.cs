...
namespace GameServer.InterestManagment
{
    public struct Grid2D<T>
    {
        ...
        readonly Dictionary<Vec2Int, HashSet<T>> grid;

        // cache a 9 neighbor grid of vector2 offsets so we can use them more easily
        readonly Vec2Int[] neighbourOffsets;

        public Grid2D(int initialCapacity)
        {
            grid = new Dictionary<Vec2Int, HashSet<T>>(initialCapacity);
        ...
        public void Add(Vec2Int position, T value)
        {
        ...
        void GetAt(Vec2Int position, HashSet<T> result)
        {
        ...
        // -> result is passed as parameter to avoid allocations
        public void GetWithNeighbours(Vec2Int position, HashSet<T> result)
        {
            // clear result first
            result.Clear();

            // add neighbours
            foreach (Vec2Int offset in neighbourOffsets)
